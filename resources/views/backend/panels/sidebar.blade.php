				...
				<li class="nav-item mr-auto">
					<a class="navbar-brand" href="{{asset('/')}}">
						...
						<h2 class="brand-text mb-0">
							{{ config('admin.templateTitle')?: 'Frest'  }}
						</h2>
					</a>
				</li>
...
@endif
{{-- horizontal-menu --}}
@if(config('admin.mainLayoutType') == 'horizontal-menu')
	<div class="header-navbar navbar-expand-sm navbar navbar-horizontal navbar-light navbar-without-dd-arrow
@if(config('admin.navbarType') === 'navbar-static') {{'navbar-sticky'}} @endif" role="navigation"
			...
			<ul class="nav navbar-nav flex-row">
				<li class="nav-item mr-auto">
					<a class="navbar-brand" href="{{asset('/')}}">
						...
						<h2 class="brand-text mb-0">
							@if(!empty(config('admin.templateTitle')) )
								{{config('admin.templateTitle')}}
							@else
								Frest
							@endif
						</h2>
...
{{-- vertical-box-menu --}}
@if(config('admin.mainLayoutType') == 'vertical-menu-boxicons')
	<div class="main-menu menu-fixed @if(config('admin.theme') === 'light') {{"menu-light"}} @else {{'menu-dark'}} @endif menu-accordion menu-shadow"
		 data-scroll-to-active="true">
		<div class="navbar-header">
			<ul class="nav navbar-nav flex-row">
